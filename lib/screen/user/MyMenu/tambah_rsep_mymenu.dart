...
class TambahRsepMymenu extends StatefulWidget {
  final String moodName; // tetap dipakai di title (opsional)
  final String? docId; // jika editing
  final Map<String, dynamic>? menuData; // harus berisi {'id': menuId, ...}

  const TambahRsepMymenu({
    super.key,
  ...
  @override
  State<TambahRsepMymenu> createState() => _TambahRsepMymenuState();
}

class _TambahRsepMymenuState extends State<TambahRsepMymenu> {
  ...
  Future<void> _loadData() async {
    // Pastikan menuData ada dan memiliki 'id'
    if (widget.menuData == null || widget.menuData!['id'] == null || widget.docId == null) return;

    setState(() => isLoadingData = true);
    try {
      final menuId = widget.menuData!['id'].toString();
      final doc = await FirebaseFirestore.instance
          .collection('MyMenu') // <-- SESUAI opsi 1
          .doc(menuId)
          .collection('resep')
          .doc(widget.docId)
          .get();
      ...
      if (doc.exists) {
        ...
        menuWarningController.text = data['tidakCocokUntuk'] ?? '';
        _imageBase64 = data['gambar'];
        steps = List<Map<String, dynamic>>.from(
          (data['langkah'] ?? []).map((s) => {
            'text': s is Map && s.containsKey('text') ? (s['text'] ?? '') : (s.toString()),
            'image': s is Map && s.containsKey('image') ? (s['image'] ?? '') : ''
          }),
        );
      }
    } catch (e) {
  ...
  Future<void> simpanData() async {
    ...
    try {
      final data = {
        ...
        'tidakCocokUntuk': menuWarningController.text.trim().isEmpty ? '-' : menuWarningController.text.trim(),
        'gambar': _imageBase64 ?? '',
        'langkah': validSteps,
